 ...
 *
 * Its instance can be accessed through `wp_lhpbpp()`. For example, if there is a plugin function called `posted_on`, it can
 * be accessed via `wp_lhpbpp()->posted_on()`.
 */
			...
			if ( ! $component instanceof Plugin_Function_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The plugin functions component %1$s does not implement the %2$s interface.', 'lhpbpp' ),
						gettype( $component ),
		...
		if ( ! isset( $this->plugin_functions[ $method ] ) ) {
			throw new BadMethodCallException(
				sprintf(
					...
					__( 'The plugin function %s does not exist.', 'lhpbpp' ),
					'wp_lhpbpp()->' . $method . '()'
				)
			);
		}
		...
		foreach ( $tags as $method_name => $callback ) {
			...
			if ( ! is_array( $callback ) || ! isset( $callback['callback'] ) ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The plugin function method %1$s registered by plugin component %2$s must either be a callable or an array.', 'lhpbpp' ),
						$method_name,
						get_class( $component )
					)
				);
			...
			if ( isset( $this->plugin_functions[ $method_name ] ) ) {
				throw new RuntimeException(
					sprintf(
						/* translators: 1: template tag method name, 2: component class name */
						__( 'The plugin function method %1$s registered by plugin component %2$s conflicts with an already registered plugin function of the same name.', 'lhpbpp' ),
						$method_name,
						get_class( $component )
					)
				);
