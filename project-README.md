...
6. Run `npm run setup`, `npm run check`, `npm start`, `npm test`, `npm run test:e2e` and `npm run release`. Install Chromium once with `npx playwright install chromium`. Confirm both ZIP names, extracted top-level directories and plugin/theme activation in WordPress.
...
The Block Editor is for editorial composition, including supported layouts. The PHP theme and its design presets control the visual system and page templates. Keep that boundary explicit in the new project's README and avoid introducing Site Editor templates by accident.
