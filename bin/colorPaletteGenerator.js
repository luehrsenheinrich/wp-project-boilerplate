...
function colorPaletteGenerator(element, meta) {
	...
	for( let className in get(meta, 'classes', {}) ) {
		...
		rule.append(new Declaration({ prop: '--current-' + prop, value: get(element, meta.valueKey) }));

		// Do we have hover rules?
		if ( get(element, 'hover', false) ) {
			rule.append(new Declaration({ prop: '--current-hover-' + prop, value: get(element, 'hover') }));
		}

		if ( get(element, 'contrast', false) ) {
			rule.append(new Declaration({ prop: '--current-contrast-' + prop, value: get(element, 'contrast') }));
		}
