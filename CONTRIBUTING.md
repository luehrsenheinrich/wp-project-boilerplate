...
4. Run `npm run check` for every code change. Run `npm test` for changes to PHP behavior, bootstrapping or WordPress integration. Run `npm run release` when changing dependencies, build configuration or packaging.
...
- Add a block's `block.json` under `plugin/blocks/<block-name>/`; the plugin's `Blocks` component discovers it. Import its editor code from `plugin/admin/src/js/blocks.js`. The current PHP renderer uses `<block-name>/template.php` when present.
...
Use a Conventional Commit style PR title such as `fix: correct theme bootstrap` or `docs: clarify project setup`. Describe the user-visible behavior, test commands and any manual checks. CI runs lint, the development build, both PHPUnit suites and production ZIP verification. Major dependency updates require individual review; Dependabot patch updates may auto-merge only after required checks pass.
